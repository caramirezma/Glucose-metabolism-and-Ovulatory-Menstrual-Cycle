...
k_I_up_est=12; % [mL/pg.d] rate in S_I (for glucose uptake)
...
% Define uniform sampling range for k_I_up
 par_min = 8;  % 1-0.33, 1+0.33
par_max = 16; 

% PGE2_min = 0.001;  % 7.92 10%
% PGE2_max = 0.008; % 9.68 10%
...
for i = 1:num_samples
  ...
  % k_I_sec= parameter_samples.k_I_sec(i);
    ...
    mi(i)=(max_Gest-max_Gout(i))/(k_I_up_est-k_I_up);
if k_I_up == 12
    ibaseline=i;
end
...
ylabel('G');
 ...
 legend('k_{I_{up}}<12','k_{I_{up}}>12');
...
for i = 1:num_samples
 ...
 max_Iout(i) = max(Iout); % Store max P
...
%% -------------------- Bottom Left: Correlation (Max I vs. k_up) --------------------
nexttile;
hold on;
 below_baseline = parameter_samples.k_I_up < 12;
%below_baseline = parameter_samples.k_I_sec < 10;
...
ylabel('I');
title('Correlation: I vs. k_{I_{up}}');
legend('k_{I_{up}}<12','k_{I_{up}}>12');
...
%% -------------------- Bottom Left: Correlation (Max B vs. k_up) --------------------
nexttile;
hold on;
  below_baseline = parameter_samples.k_I_up< 12;
%below_baseline = parameter_samples.k_I_sec < 10;
...
title('Correlation: B vs. k_{I_{up}}');
grid on;
legend('k_{I_{up}}<12','k_{I_{up}}>12')
